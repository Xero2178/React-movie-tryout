...
export default function App() {
  ...
  function handleRemoveFavMovie(favMovie) {
    setAddFavorite((addFavorite) =>
      addFavorite.filter((movie) => movie.imdbID !== favMovie.imdbID)
    );
  }
  ...
  return (
    <div className="flex justify-center items-center flex-col m-5 relative">
      ...
      <Favorites
        ...
        onAddToFavorite={handleAddFavorite}
        onRemoveFavorite={handleRemoveFavMovie}
      />
    </div>
  );
}
...
function Header({ sort, onSort, name, onSetName, onClearName, onSetFind}) {
  return (
    <form className="max-w-2xl flex justify-around items-center w-full">
      ...
      <input
        type="text"
        className="border-4 border-black rounded-xl px-4"
        placeholder="Search"
        value={name}
        onChange={(e) => onSetName(e)}
      />
      <span className="text-2xl font-bold text-red-700 cursor-pointer" onClick={() => onClearName()}>&times;</span>
    </form>
  );
}
  ...
  return (
    <div className="text-black font-bold text-center text-2xl border-black border-2 ">
      ...
      <div>
        <Button
          className={
            "text-black text-center border-2 border-black text-sm w-full bg-emerald-600 "
          }
          onClick={() => onAddToFavorite(movie)}
        >
          Add to favorites
        </Button>
...
function AboutMovie({ about, onSetAbout }) {
  ...
  return (
    ...
    >
      <div
        className="text-4xl font-extrabold text-right cursor-pointer"
        onClick={() => onSetAbout(null)}
      >
        &times;
      </div>
      <img
      ...
      />
      <h2 className="bg-white w-full text-center font-bold text-2xl">
        {about.Title}
      </h2>
...
function Favorites({ addFavorite, onAddToFavorite, onRemoveFavorite }) {
  ...
  return (
    <div>
      ...
      <div className="movie-theatre grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-w-5xl xl:grid-cols-5 m-5">
        {addFavorite.slice().map((favMovie) => (
          <FavoriteMovie
            favMovie={favMovie}
            onAddToFavorite={onAddToFavorite}
    ...
    <div className="text-black font-bold text-center text-2xl border-black border-2 ">
      ...
      <div>
        <Button
          ...
          onClick={() => onRemoveFavorite(favMovie)}
        >
