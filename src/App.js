...
export default function App() {
  ...
  function handleAddFavorite(movie) {
    ...
    const isFav = addFavorite.find((favMovie) => favMovie.imdbID === movie.imdbID)

    if(!isFav) {
      setAddFavorite((addFavorite) => [...addFavorite, movie])
    }

    console.log(!isFav);

    
    // console.log(addFavorite.map((favMovie) => favMovie.imdbID.toString()));
    // console.log(movie.imdbID);

    // setAddFavorite([...addFavorite, movie])
    
   
  }

  function handleRemoveFavMovie(favMovie) {

    setAddFavorite((addFavorite) => addFavorite.filter((movie) => movie.imdbID !== favMovie.imdbID))
  }
  ...
  return(
    <div className="flex justify-center items-center flex-col m-5 relative">
      ...
      <Header sort={sort} onSort={handleSort} search={search} onSearch={handleSearch}/>
      {about && <AboutMovie onSetAbout ={handleSetAbout} about={about}/>}
      <MovieTheatre onAddToFavorite = {handleAddFavorite} sort={sort} search={search} onSetAbout ={handleSetAbout} about={about}/>
      <Favorites addFavorite={addFavorite} onAddToFavorite = {handleAddFavorite} onRemoveFavorite={handleRemoveFavMovie}/>
    </div>
  )

}

function Header({sort, onSort, search, onSearch}) {
  ...
  return(
    <form className="max-w-2xl flex justify-around items-center w-full" onSubmit={(e) => handleSubmit(e)}>
      <select className="border-2 border-black rounded-xl px-2" value={sort} onChange={(e) => onSort(e)}>
        <option value={1}>Sort Alphabetically</option>
        <option value={2}>Sort by Rating</option>
        <option value={3}>Sort by Released date</option>
      </select>
      <input type="text" className="border-4 border-black rounded-xl px-4" placeholder="Search" value={name} onChange={(e) => handleSetName(e)}/>
      <Button className={'text-black text-lg px-2 border-2 border-black rounded-lg'}>SEARCH</Button>

    </form>
  )
}

function MovieTheatre({onAddToFavorite, search, about, onSetAbout}) {

  const isSelected = movieData.find((movie) => movie.Title === search ? <Movie onSetAbout ={onSetAbout} about={about}/> : null)
  ...
  if(!isSelected) {
    ...
    return(
      <div className="movie-theatre grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-w-5xl xl:grid-cols-5 m-5">
        {movieData.map((movie) => movie.Title === search ? <Movie movie={movie} onAddToFavorite={onAddToFavorite} onSetAbout ={onSetAbout} about={about}/> : null)}
      </div>
    )
  }
}

function Movie({movie, onAddToFavorite, about, onSetAbout}) {

  function handleMovieDetails(e) {
  ...
  return(
    <div className="text-black font-bold text-center text-2xl border-black border-2 ">
      <img src={movie.Images[0]} alt="movie.Title" className="w-full h-[130px]" onClick={(e) => handleMovieDetails(e)}/>
      <div className="movie-title">{movie.Title}</div>
      <div>
        <Button className={'text-black text-center border-2 border-black text-sm w-full bg-emerald-600 '} onClick={() => onAddToFavorite(movie)}>Add to favorites</Button>
        {/* <Button className={'text-black text-center border-2 border-black text-sm'}>Add to favorites</Button> */}
      </div>
    </div>
  )
}

function AboutMovie({about, onSetAbout}) {

  console.log(about);
  return(
    <div className={`${about ? 'absolute z-10 top-[100px] w-full h-full bg-white' : 'hidden'}`}>
      <div className="text-4xl font-extrabold text-right cursor-pointer" onClick={() =>(onSetAbout(null))}>&times;</div>
      <img src={about.Images[0]} alt={about.Title} className="w-full h-[420px]"/>
      <h2 className="bg-white w-full text-center font-bold text-2xl">{about.Title}</h2>
      <p className="bg-white w-full">{about.Plot}</p>
    </div>
  )
}
...
function Favorites({addFavorite, onAddToFavorite,onRemoveFavorite}) {
  ...
  return (
    <div>
      <div className="text-3xl font-bold">FAVORITES</div>
      <div className="movie-theatre grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-w-5xl xl:grid-cols-5 m-5">
        {addFavorite.slice().map((favMovie) => <FavoriteMovie favMovie={favMovie} onAddToFavorite={onAddToFavorite} onRemoveFavorite={onRemoveFavorite} addFavorite={addFavorite}/>)}
      </div>
    </div>
  )
}

function FavoriteMovie({favMovie, onRemoveFavorite}) {
  ...
  return(
    <div className="text-black font-bold text-center text-2xl border-black border-2 ">
      <img src={favMovie.Images[0]} alt="favMovie.Title" className="w-full h-[130px]" />
      <div className="movie-title">{favMovie.Title}</div>
      <div>
        
        <Button className={'text-black text-center border-2 border-black text-sm w-full bg-emerald-600 '} onClick={() => onRemoveFavorite(favMovie)}>Remove from favorites</Button>
      </div>
    </div>
  )
}
